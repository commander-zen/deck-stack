...
window.addEventListener("error", (e) => {
  if (rootEl && !rootEl._reactRootContainer && rootEl.childElementCount === 0) {
    ...
    rootEl.innerHTML = `<div style="max-width:400px;font-family:'Noto Sans',sans-serif;color:#ff4d6d"><div style="font-size:20px;margin-bottom:12px">LOAD ERROR</div><pre style="font-size:12px;white-space:pre-wrap;word-break:break-word;color:#e8e8f0">${String(e.error?.message ?? e.message)}\n\n${String(e.error?.stack ?? "")}</pre></div>`;
  }
});
